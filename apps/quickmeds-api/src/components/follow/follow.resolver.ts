...
export class FollowResolver {
	...
	public async subscribe(@Args('input') input: string, @AuthMember('_id') memberId: ObjectId): Promise<Follower> {
		...
		const followingId = shapeIntoMoongoObjectId(input);
	...
	public async getMemberFollowings(
		...
		console.log('Query: getMembersFollowings');
		const { followerId } = input.search;
		input.search.followerId = shapeIntoMoongoObjectId(followerId);
	...
	public async getMemberFollowers(
		...
		console.log('Query: getMembersFollowers');
		const { followingId } = input.search;
		input.search.followingId = shapeIntoMoongoObjectId(followingId);
